...
public class SaveContact extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idPs != null) {
            ...
            if (request.getParameter("cn").trim().equals("") || request.getParameter("t").trim().equals("")) {
                response.sendRedirect("contacts.jsp?id=" + idPs);
                return;
            } else {
            ...
            if (idPs != null && idCs != null) {
                ...
                nc.setId(idC);
                for (Contact c : DB.getContacts(em, p.getId())) {
                    if (idC.equals(c.getId())) {
                        DB.updateContact(em, nc);
                        System.out.println("nc: " + nc);
                    }
                }
            } else {
                DB.addContact(em, p.getId(), nc);
            }
        }
        response.sendRedirect("contacts.jsp?id=" + idPs); 
    }
